...
if (isset($_POST['edit_submit'])) {
  ...
  if (!empty($_FILES['flat_img_internal']['name'])) // image not empty but updated
  { //not empty
    if (move_uploaded_file($file_temp_loc1, $file_store1)) {
      $sql_update = "UPDATE `flat` SET a_id='$a_id',`flat_owner`='$flat_owner',`pincode`='$pincode',`flat_rate`='$flat_rate',`sq_ft`='$sq_ft',`property_type`='$property_type',`flat_img_internal`='$image1',`flat_type`='$flat_type',`status`='$status' WHERE id='$id'";
    }
  } else // no image updated
  {
    $sql_update = "UPDATE `flat` SET a_id='$a_id', `flat_owner`='$flat_owner',`pincode`='$pincode',`flat_rate`='$flat_rate',`sq_ft`='$sq_ft',`property_type`='$property_type',`flat_type`='$flat_type',`status`='$status' WHERE id='$id'";
  }
      ...
      <select class="form-control" name="a_id">
       ...
       <?php
              ...
              while ($row2 = $result2->fetch_assoc()) {
                //echo $row['a_id'];
                //exit();
                if ($row2['a_id1'] == $row['a_id']) {
                  echo "<option value= " . $row2['a_id1'] . " selected>" . $row['a_id'] . "</option>";
                } else {
                  echo "<option value= " . $row2['a_id1'] . ">" . $row2['a_id1'] . "</option>";
                }
              ?>
       <?php
              }
